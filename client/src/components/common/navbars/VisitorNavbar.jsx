...
import { Search, ShoppingBag, Menu, X, ArrowRight, User, Bell } from "lucide-react";
import { Button } from "@/components/ui/button";
import { getMediaUrl } from "@/lib/mediaUrl";
import Logo from '@/assets/virtual_exhibit_logo.png';
import OriginalLogo from '@/assets/original_logo.jpg';

// Renders the navigation bar for authenticated visitors, including personal collection links and user controls
export function VisitorNavbar({ forceDark = false }) {
    ...
    const navLinks = [
        { name: "Exhibitions", path: "/categories" },
        { name: "My Collection", path: "/dashboard/visitor" },
        { name: "Contact", path: "/contact" },
    ];

    const shouldUseDark = isScrolled || forceDark;
    const textColorClass = shouldUseDark ? "text-black" : "text-white";
            ...
            >
                <div className="container mx-auto px-6 md:px-12 flex items-center justify-between">
                    ...
                    <div className="hidden md:flex items-center gap-6">
                        ...
                        <div className={`flex items-center gap-4 border-r pr-6 transition-colors duration-300 group-hover:border-gray-800 ${borderColorClass}`}>
                            <button className={`transition-all duration-300 hover:opacity-70 group-hover:text-white ${textColorClass}`}>
